...
std::vector<MNIST> create_MNIST_testing_set(MNIST_dataset<Pixel, Label> dataset)
{
	std::vector<MNIST> mnist(std::size(dataset.test_images));
	std::transform(std::begin(dataset.test_images), std::end(dataset.test_images), std::begin(dataset.test_labels), std::begin(mnist), [&] (auto & ti, auto & tl) {
		MNIST mnist;
		mnist.in.resize(784);
		std::transform(std::begin(ti), std::end(ti), std::begin(mnist.in), [&] (auto & i) {
			return i;
		});
		mnist.out.resize(10);
		mnist.out.at(size_t(tl)) = 1.0;
		return mnist;
	});
	return mnist;
};
...
template <typename Pixel, typename Label>
std::vector<MNIST> create_MNIST_training_set(MNIST_dataset<Pixel, Label> dataset)
{
	std::vector<MNIST> mnist(std::size(dataset.training_images));
	std::transform(std::begin(dataset.training_images), std::end(dataset.training_images), std::begin(dataset.training_labels), std::begin(mnist), [&] (auto & ti, auto & tl) {
		MNIST mnist;
		mnist.in.resize(784);
		std::transform(std::begin(ti), std::end(ti), std::begin(mnist.in), [&] (auto & i) {
			return i;
